...
- **Truth lives in `~/.local/share/copilot-env`** (`src/copilot_api/paths.ts`): nothing outside it is ever read as truth. Agent files (`config.toml`, `settings.json`, the Claude Desktop config) are outputs: a command such as `agent init` or `--sync` re-renders them from that state and corrects a stale or hand-edited copy, and nothing rewrites them without a command.
