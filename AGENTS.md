...
- **Supply-chain cooldown applies to the installers too.** `install.sh --cooldown` / `install.ps1 -Cooldown` install the newest copilot-env *release* that is ≥7 days old (a full clone, then check out the aged tag) **and** the newest agent-CLI (claude/copilot/codex) npm releases that old. The CLI resolver `src/install/aged-version.ts` is shared verbatim by both installers; the repo aged-release pick (`resolve_aged_release` / `Resolve-AgedRelease`) is the per-installer shell mirror, kept feature-matched.
...
- **No new deps without an explicit reason.** Current: `citty`, `consola`, `smol-toml`, `tar` (release-tarball extraction for `agent update`), `@jeffreycao/copilot-api`, `patch-package`.
