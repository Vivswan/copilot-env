...
- **No new deps without an explicit reason.** Current: `citty`, `consola`, `dotenv` (root `.env` loading), `execa` (subprocess spawning), `smol-toml`, `tar` (release-tarball extraction for `agent update`), `which` (resolve executables on PATH), `ps-list` (process enumeration), `@jeffreycao/copilot-api`, `patch-package`.
