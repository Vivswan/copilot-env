...
- The pin follows no `gh auth switch`; it fails only when gh cannot serve that account (logged out, or gh older than 2.40). An env token (`$GH_TOKEN`, `$GITHUB_TOKEN`) is never adopted for a pin, and a miss quotes the gh call and its stderr.
