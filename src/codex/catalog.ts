 ...
 */
export const CATALOG_PATCH_VERSION = 2;
 ...
 * 1.05M window would 413 upstream before Codex ever auto-compacts. Service-tier
 * advertisements (`service_tiers`, `additional_speed_tiers`) are stripped from
 * EVERY model: they make Codex send `service_tier` on requests, which Copilot's
 * /responses endpoint rejects outright, and this catalog only ever wires Codex
 * at Copilot. Everything else (fields and non-matching models) is preserved
 * verbatim. Returns null on any parse problem so a bad dump can never produce a
 * catalog Codex rejects.
 */
