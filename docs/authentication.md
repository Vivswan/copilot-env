...
- The pin stays honest: a `gh auth switch` to another account fails the pin instead of following it, an env token (`$GH_TOKEN`, `$GITHUB_TOKEN`) is never adopted for a pin, and a miss quotes the gh call and its stderr.
