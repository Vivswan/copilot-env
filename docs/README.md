...
| Understand Direct vs the proxy, and which one `agent profile add` picks     | [Getting started: the first `agent profile add`](getting-started.md#the-first-agent-profile-add) |
...
1. [Getting started](getting-started.md) - install and its flags, the first `agent profile add` and the two modes, updating, verifying a download, uninstall.
