...
- `test/fake-gateway.mjs` — stand-in gateway for the start/stop check: binds the port, answers the admin endpoints, prints the `Listening on:` marker, stays alive until `SIGTERM`. Selected via the `COPILOT_API_ENTRY` override in `copilot_api/process.ts:resolveCopilotApiEntry`
