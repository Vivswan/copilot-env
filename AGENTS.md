...
- `src/codex/` — Codex config, per-host `CODEX_HOME` farm logic, the `--mobile` phone-pairing flow (`mobile.ts`: temporarily strips `model_provider` around the Codex app's remote-control pairing), the config-free desktop-app control layer (`app_control.ts`: detect/open/quit, shared by `mobile.ts` + `app_patch.ts`), and the desktop-app `image_generation` patch (`app_patch.ts`: best-effort macOS/Windows edit of the app's `app.asar` so the app's image-gen gate tracks Direct vs Proxy — see the Codex note below).
...
- **No new deps without an explicit reason.** Current: `commander` (CLI parsing; rejects unknown flags + native help wrapping), `consola`, `dotenv` (root `.env` loading), `execa` (subprocess spawning), `smol-toml`, `tar` (release-tarball extraction for `agent update`), `which` (resolve executables on PATH), `ps-list` (process enumeration), `@electron/asar` (read/repack the Codex desktop app's `app.asar` for the macOS/Windows `image_generation` patch — see the Codex note), `@jeffreycao/copilot-api`, `patch-package`.
...
## Codex desktop-app `image_generation` patch

`configureCodexConfig` writes `[features] image_generation = false` into `config.toml` for **Direct** and drops it for **Proxy** — but the Codex *desktop app* gates that feature in its own bundled JS and ignores `config.toml`. `src/codex/app_patch.ts` closes that gap: `runCodex`/`runCodexHost` call `syncCodexAppImageGeneration(direct)` after writing config to edit the app's `app.asar` (via `@electron/asar`) so the app's gate tracks the mode — Direct patches it out, Proxy restores it.

It is **best-effort and idempotent**: a failure only warns (the `config.toml` write is the real contract), and a no-op when already in the desired state — so it's safe to call on every `agent codex`. Set `COPILOT_API_NO_APP_PATCH=1` to opt out entirely (also how the test suite avoids patching a real installed app — `test/setup.ts`, wired via bunfig `[test].preload`). macOS + Windows only (no Linux app). The patch is a **bidirectional string transform** (`patchImageGenerationSource`/`unpatchImageGenerationSource`, the only unit-tested part) that captures the minified identifiers with backreferences rather than hardcoding them, so it survives identifier churn — but **not a structural rewrite**, in which case it warns and skips. It also **skips rather than risk bricking** when the app uses asar-integrity (macOS `Info.plist` `ElectronAsarIntegrity`) or has unpacked native files, and on Windows when the app is running (locked `app.asar`) or installed as a read-only `WindowsApps` package. **Caveat:** modifying `app.asar` breaks the bundle's code signature; the anchor needs re-verifying when a Codex release moves it (the idempotent re-apply on the next `agent codex` handles a reverted patch). The config-free app-control helpers (`open`/`quit`/`isRunning`) live in `src/codex/app_control.ts`, shared with `mobile.ts`, so neither importer creates a cycle through `config.ts`.
