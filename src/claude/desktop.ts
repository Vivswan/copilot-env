...
// inferenceModels list derived from the live catalog (static fallback offline); the local
 ...
 *  (no separators, no leading dot/underscore -- `_meta` itself can never be an id), so
 *  a hand-mangled id can neither escape the library dir nor collide with _meta.json. */
...
export function parseDesktopMeta(raw: string | null): DesktopMeta | null {
  ...
  if (doc.appliedId !== undefined && !SAFE_ENTRY_ID.test(doc.appliedId)) return null;
  ...
  for (const entry of Array.isArray(doc.entries) ? doc.entries : []) {
    ...
    if (!SAFE_ENTRY_ID.test(entry.id)) return null;
...
/** Offline fallback when the live catalog is unreachable: the current frontier family
 *  defaults. RAW catalog ids -- external contracts, like DEFAULT_WEB_SEARCH_MODEL. */
export const DESKTOP_FALLBACK_MODELS: readonly DesktopModelSpec[] = [
  {
    name: "claude-fable-5",
    supports1m: true,
    prefer1m: true,
...
export function desktopConfigPayload(opts: DesktopPayloadOptions): Record<string, unknown> {
  ...
  doc["inferenceModels"] = (opts.models ?? DESKTOP_FALLBACK_MODELS).map((m) => ({
    "name": m.name,
...
/** Map catalog family picks onto Desktop model rows (every pick a family default). */
export function desktopModelsFromPicks(picks: ClaudeFamilyPick[]): DesktopModelSpec[] {
  return picks.map((p) => ({
    name: p.id,
    supports1m: p.is1m,
    prefer1m: p.is1m,
    anthropicFamilyTier: p.family,
    isFamilyDefault: true,
  }));
}
 ...
 *  adoption iff the adopted entry actually referenced it. */
function legacyHandMadeHelperPath(): string {
  return join(resolveClaudeHome(), "copilot-token.sh");
}

/** Union `models` with the fallback entries for families the catalog omitted: Copilot
 *  serves some frontier ids on inference (claude-fable-5) WITHOUT advertising them in
 *  /models, and the picker must not lose them. Catalog picks win per family. */
export function withFallbackFamilies(models: DesktopModelSpec[]): DesktopModelSpec[] {
  const families = new Set(models.map((m) => m.anthropicFamilyTier));
  return [
    ...models,
    ...DESKTOP_FALLBACK_MODELS.filter((m) => !families.has(m.anthropicFamilyTier)),
  ];
}

/** The model rows for a wiring: live catalog picks, else the static fallback. Proxy
 *  prefers the daemon's own aggregated catalog (exactly what /v1/models will return)
 *  and falls back to the direct Copilot catalog when the daemon is down -- the rows are
 *  the 1m-capability annotations discovery alone cannot provide (claude-code#88345). */
async function wiringModels(opts: DesktopWireOptions): Promise<readonly DesktopModelSpec[]> {
  const sources: CatalogSource[] = opts.mode === "proxy" ? ["proxy", "direct"] : ["direct"];
  for (const source of sources) {
    try {
      ...
      const picks = claudeFamilyPicks(parseCatalogModels(body));
      if (picks.length > 0) return withFallbackFamilies(desktopModelsFromPicks(picks));
    } catch (e) {
      logger.warn(
  ...
  logger.warn("  Claude Desktop: using the static fallback model list.");
  return DESKTOP_FALLBACK_MODELS;
}
...
export async function wireClaudeDesktopEntry(opts: DesktopWireOptions): Promise<void> {
  ...
  if (entry === undefined) {
    ...
    for (const candidate of nameTaken ? [] : meta.entries) {
      ...
      if (doc["inferenceCredentialHelper"] === handMade) {
  ...
  // Model rows: the launcher hot path (quiet refresh of an already-owned entry)
  // reuses the recorded rows instead of paying a catalog fetch per launch --
  // init / profile-add / `agent claude --desktop` (quiet=false or fresh entries)
  // always refresh from the live catalog.
  const recordedModels = owned && opts.quiet ? recordedModelRows(existing) : null;
  const models = recordedModels ?? await wiringModels(opts);
  ...
  if (!opts.quiet) {
    logger.success(
      `  Claude Desktop: wired "${name}" (${opts.mode}); restart Claude Desktop to pick it up.`,
    );
  }
}
...
export function removeClaudeDesktopEntry(profile: Profile): void {
  try {
    removeOwnedEntries((entry, owned) => owned && entry.name === desktopEntryName(profile));
    const rootHome = resolveRootHome();
...
export function removeAllClaudeDesktopWiring(dirOverride?: string | null): void {
  removeOwnedEntries((_entry, owned) => owned, dirOverride);
...
function removeOwnedEntries(
  ...
  if (raw === null) return; // no library, nothing recorded here to remove
