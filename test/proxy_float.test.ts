...
/** Write a fake installed proxy at the given version. */
function installProxy(root: string, version: string): void {
...
function spawnWithTime(timeMap: Record<string, string>): {
  ...
  return {
    ...
    "spawn": (_command, args) => {
      ...
      if (args[0] === "add") {
        const spec = args[1] ?? "";
        installProxy(dir, spec.slice(`${PROXY_PKG}@`.length));
      }
      return SPAWN_OK;
    },
  };
...
describe("proxyFloatUpToDate", () => {
  test("true for normal no-env float when the computed cooldown-aged target is installed", () => {
    ...
    expect(proxyFloatUpToDate(dir, "bun", CONFIG, 604800, spawn, NOW_MS)).toBe(true);
    expect(calls[0]).toEqual(["pm", "view", PROXY_PKG, "time", "--json"]);
  });
  ...
  test("false when the proxy is not installed", () => {
    expect(proxyFloatUpToDate(dir)).toBe(false);
  });
  ...
  test("COPILOT_API_MIN_RELEASE_AGE overrides the cooldown window (0 = no cooldown)", () => {
    ...
    expect(
      proxyFloatVerifyStatus(dir, "bun", CONFIG, undefined, spawnWithTime(times).spawn, NOW_MS)
        .upToDate,
    ).toBe(false);
  ...
  test("COPILOT_API_VERSION: true only when the exact pin is installed", () => {
    ...
    expect(proxyFloatUpToDate(dir)).toBe(true);
...
describe("floatProxy", () => {
  test("default no-env float installs the exact newest cooldown-aged target", () => {
    ...
    floatProxy(dir, "bun", CONFIG, 604800, spawn, NOW_MS);
  ...
  test("metadata failure keeps an installed proxy that satisfies the floor", () => {
    installProxy(dir, "1.10.30");
  ...
  test("metadata failure pins the floor when no installed proxy satisfies it", () => {
    const { calls, spawn } = spawnWithMetadataFailure();
