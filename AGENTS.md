...
- **Direct mode needs a Copilot GitHub OAuth token, normally via `gh`.** By default the Direct configs mint their bearer by shelling out to `gh auth token` (Codex provider `auth.command`, Claude `apiKeyHelper`), so Direct depends on an authenticated `gh` binary. `--gh-token [token]` (on `init` / `codex` / `claude` / `setup-clis`) removes that dependency for headless servers: it bakes the token into the Direct configs (Claude apiKeyHelper prints it; Codex reads it from `.env` via a dedicated `env_key` = `COPILOT_ENV_GH_TOKEN` — deliberately not `GITHUB_TOKEN`/`GH_TOKEN`, so it can't shadow the machine's real `gh`/`git` account — no `auth` block) **and** stores it in copilot-env's per-host state so `agent start` passes it to the daemon as `--github-token`. copilot-api uses that token in-memory only — it never writes it to its own `github_token` file, so an existing device-flow login is left untouched. A bare `--gh-token` reads `$GH_TOKEN`/`$GITHUB_TOKEN`. The token must be a Copilot-enabled OAuth token (what `gh auth token` emits) — a fine-grained PAT has no Copilot access. **`agent init --remove-gh-token`** reverts: reconfigures both agents to gh-direct (scrubbing the baked Codex `.env` token) and clears the stored proxy token.
