...
`agent config` prints the store under two banners. `PROFILE <name>` holds the selected profile's own keys and any
`profile-default` value it overrides (naming the value it hides). `GLOBAL` holds the machine's keys, the `proxy` group
among them as every profile's default.
