...
Web search for AI coding agents through GitHub Copilot's Responses API, backed by the copilot-env MCP server (`agent profile mcp --serve`).
...
The server ships with [copilot-env](https://github.com/Vivswan/copilot-env); `agent profile add` / `agent profile sync` wire it into Claude Code automatically on a Direct write. To register it anywhere else (in Claude Code specifically, pick a name other than `copilot-env` - copilot-env's own wiring writes reclaim that name when switching modes):
...
On Windows, run the PowerShell launcher instead: `powershell -NoProfile -ExecutionPolicy Bypass -File <checkout>/bin/agent.ps1 profile mcp --serve`.
...
The server resolves the copilot-env credential (`agent profile auth`). Without one it reads `COPILOT_GITHUB_TOKEN`, `GH_TOKEN`, or `GITHUB_TOKEN` from the environment, so a bare clone works with `GH_TOKEN=... bin/agent profile mcp --serve`. The registered server always uses the default credential; a named profile that needs its own can register a second entry running `agent profile <name> mcp --serve`.

## Configuration

- `agent config set proxy.message-websearch-model <id>` picks the search model (read on every call; default `gpt-5-mini`, the same default the proxy uses).
- `agent profile mcp --remove` unregisters the server from Claude Code, restores the builtin WebSearch permission, and stores a durable opt-out (`claude.wire-mcp false`).
