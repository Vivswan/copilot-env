...
- **Cells:** `accepted (N models)` is a 2xx with the `/models` catalog size; `rejected (400)` a 400/401; `unclear (network error)` a blip; `-` an identity not probed on that host, because it is neither one of the host's candidates nor pinned nor baked there.
