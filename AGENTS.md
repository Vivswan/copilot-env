...
- **Direct mode needs a Copilot GitHub OAuth token, normally via `gh`.** By default the Direct configs mint their bearer by shelling out to `gh auth token` (Codex provider `auth.command`, Claude `apiKeyHelper`), so Direct depends on an authenticated `gh` binary. **`agent init --gh-token [token]`** removes that dependency for headless servers. The token is the **single source of truth**, stored in a SHARED file under the copilot-api home (`~/.local/share/copilot-api/.copilot-env-state.json`, NOT per-host `.run/` state — the token is account/machine-wide) via `src/copilot_api/gh_token.ts`. Resolution order everywhere: an explicit `--gh-token` (written to the store) → the stored token → the `gh` CLI. Every consumer reads `readStoredGithubToken()`: when a token is present, `runClaude`/`runCodex`/`runCodexHost` select Direct (no probe) and bake it in (Claude's `apiKeyHelper` prints it; Codex's `github-copilot-direct` provider reads it from `.env` via a dedicated `env_key` = `COPILOT_ENV_GH_TOKEN` — deliberately not `GITHUB_TOKEN`/`GH_TOKEN`, so it can't shadow the machine's real `gh`/`git` account — with no `auth` block), and `agent start` passes it to the daemon as `--github-token` (in-memory only — copilot-api never writes it to its own `github_token` file, so an existing device-flow login is untouched). `--proxy`/`--direct` still override the mode (the token is then only used by the proxy). A bare `--gh-token` reads `$GH_TOKEN`/`$GITHUB_TOKEN`. The token must be a Copilot-enabled OAuth token (what `gh auth token` emits) — a fine-grained PAT has no Copilot access. **`agent init --remove-gh-token`** reverts: reconfigures both agents to gh-direct (scrubbing the baked Codex `.env` token) and clears the stored token.
...
- `src/commands/init.ts` — `agent init`: configure BOTH agents (auto/--direct/--proxy) + next-step guidance; owns `--gh-token`/`--remove-gh-token`. Exports `configureBothAgents`.
...
- **No new deps without an explicit reason.** Current: `commander` (CLI parsing; rejects unknown flags + native help wrapping), `consola`, `dotenv` (root `.env` loading), `execa` (subprocess spawning), `smol-toml`, `tar` (release-tarball extraction for `agent update`), `which` (resolve executables on PATH), `ps-list` (process enumeration), `@jeffreycao/copilot-api`, `patch-package`.
